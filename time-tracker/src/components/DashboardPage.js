...
const DashboardPage = () => {
  ...
  return (
    <Box sx={{ minHeight: '100vh', width: '100vw', background: 'linear-gradient(to bottom right, #eff1f3, #ffffff)', py: 4 }}>
      <Container maxWidth="md">
        <Card sx={{ mb: 4, boxShadow: 4 }}>
          <CardContent>
            <Typography variant="h5" align="center" gutterBottom>ยินดีต้อนรับ</Typography>
            <Box sx={{ my: 2 }}>
              <Typography>เหลือเวลาอีก: {formatTime(remainingTime)}</Typography>
            </Box>
            <Button variant="contained" color="primary" fullWidth disabled={!isLogoutEnabled}>ออกงาน</Button>
          </CardContent>
          <CardActions sx={{ justifyContent: 'center' }}>
            <Button variant="outlined" onClick={() => navigate('/')}>กลับไปหน้าแรก</Button>
          </CardActions>
        </Card>
      </Container>
    </Box>
